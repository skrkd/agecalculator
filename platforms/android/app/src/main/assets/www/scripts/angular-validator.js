        ...
        return {
            ...
            link: function (scope, element, attrs, ctrl) {
                ...
                attrs.$observe('validator', function (value) {
                    ...
                    if (match) {
                        rule = $validator.convertRule('dynamic', {
                            ...
                            invoke: (attrs.validatorInvoke === undefined ? "blur" : attrs.validatorInvoke),
                            error: (attrs.validatorError === undefined ? "Invalid Input!" : attrs.validatorError)
                        });
                        rules.push(rule);
                        return;
                    }
                ...
                isAcceptTheBroadcast = function (broadcast, modelName) {
                    ...
                    //if (!$(element).is(':visible') || $(element).is(':disabled')) // added by mitan for ignoring disabled and hidden controls
                    if ($(element).is(':disabled')) // added by mitan for ignoring disabled and hidden controls
                        return false;
                    if (!$(element).is(':visible') && ($(element).parents('[ng-show]').length > 0 || $(element).parents('[ng-hide]').length > 0))
                        return false;
                    if (modelName) {
        ...
        this.convertError = function (error) {
            ...
            */
            ...
            return function (value, scope, element, attrs) {
                ...
                if (element.attr("validator-message") !== undefined)
                    errorMessage = element.attr("validator-message");
                else if (this.customError !== undefined || this.customError === 'function')
                    errorMessage = this.customError(attrs);

                $(element).parents('.panel').find('i.collapsed').click();
                parent = $(element).parent();
                if (!element.hasClass("has-error"))
                    element.addClass('has-error');

                $(element).attr("data-toggle", "tooltip").attr("title", errorMessage);
